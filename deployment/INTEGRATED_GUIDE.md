...
- **Laravel**: `/var/www/html/metapilot`
- **Crawler Service**: `/var/www/html/crawler-service`

## 2. Setting Up the Python Crawler Service
1.  **Move the folder**: Upload `crawler-service` to `/var/www/html/crawler-service`.
2.  **Run Setup**:
    ```bash
    cd /var/www/html/crawler-service
    chmod +x setup_crawler_production.sh # Copy this from the deployment folder I created
    ./setup_crawler_production.sh
    ```
3.  **Configure `.env`**: Create a `.env` in the crawler folder:
    ```env
...
3.  **Update Paths**: Edit the `.conf` files to ensure they point to `/var/www/html/metapilot` and `/var/www/html/crawler-service` respectively.
4.  **Activate**:
    ```bash
