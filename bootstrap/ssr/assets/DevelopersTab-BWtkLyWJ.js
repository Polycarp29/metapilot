...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<svg class="${ssrRenderClass([{ "rotate-180": showSiteDropdown.value }, "w-3 h-3 transition-transform"])}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M19 9l-7 7-7-7"></path></svg></button>`);
      if (selectedSite.value) {
        _push(`<button class="w-8 h-8 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-400 hover:text-indigo-600 transition-all shadow-sm"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path></svg></button>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<button class="px-3 py-2 text-slate-400 hover:text-white hover:bg-slate-900 rounded-xl transition-all"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 4v16m8-8H4"></path></svg></button></div>`);
      ...
      ssrRenderList(topPages.value, (page, idx) => {
        ...
        if (page.showRecs) {
          ...
          _push(`<div class="mt-6 flex items-center gap-4"><button class="px-4 py-2 bg-indigo-600 text-white text-[9px] font-black rounded-lg uppercase tracking-tight shadow-md hover:indigo-500 transition-all active:scale-95">Auto-Inject Schema</button><button class="px-4 py-2 bg-white text-slate-600 border border-slate-200 text-[9px] font-black rounded-lg uppercase tracking-tight hover:bg-slate-50 transition-all active:scale-95">View Page Source</button></div></div></div></div></td></tr>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<!--]-->`);
      ...
      _push(`<!--]--></div></div></div><div class="grid grid-cols-1 lg:grid-cols-12 gap-8"><div class="flex items-center justify-between mb-10"><div class="flex items-center gap-5"><div class="p-4 bg-indigo-600 text-white rounded-2xl shadow-lg relative"><svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"></path></svg>`);
      if (selectedSite.value) {
        _push(`<span class="${ssrRenderClass([{
          "bg-emerald-500": selectedSite.value.status === "verified_active",
          "bg-amber-400": selectedSite.value.status === "connected_inactive",
      ...
      _push(`</div></div></div><div class="flex items-center gap-4"><button class="px-5 py-3 bg-white/5 border border-white/10 text-white text-[10px] font-black rounded-2xl flex items-center gap-2 hover:bg-white/10 transition-all shadow-xl"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 4v16m8-8H4"></path></svg> New Identification </button></div></div><div class="grid grid-cols-1 md:grid-cols-12 gap-6 mb-8"><div class="md:col-span-4 space-y-3"><label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Traffic Origin (Pinning)</label><div class="relative group/input"><input${ssrRenderAttr("value", allowedDomainInput.value)} placeholder="e.g. site.com" class="w-full bg-slate-800 border-white/10 focus:border-indigo-500 focus:ring-0 rounded-2xl text-[11px] font-bold text-white py-4 px-6 pr-14"><button class="absolute right-3 top-2.5 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-[9px] font-black rounded-lg opacity-0 group-hover/input:opacity-100 transition-all"> Save </button></div></div><div class="md:col-span-4 space-y-3"><label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Lead Attribution ID</label><input${ssrRenderAttr("value", selectedCampaignId.value)} placeholder="e.g. fb_ads_winter_campaign" class="w-full bg-white/5 border-white/10 focus:border-indigo-500 focus:ring-0 rounded-2xl text-[11px] font-bold text-white py-4 px-6"></div><div class="md:col-span-4 space-y-3"><label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Identification Source</label><select class="w-full bg-slate-800 border-white/10 focus:border-indigo-500 focus:ring-0 rounded-2xl text-[11px] font-bold text-white py-4 px-6 appearance-none cursor-pointer"><option${ssrRenderAttr("value", null)}${ssrIncludeBooleanAttr(Array.isArray(selectedSiteId.value) ? ssrLooseContain(selectedSiteId.value, null) : ssrLooseEqual(selectedSiteId.value, null)) ? " selected" : ""}>System Selection...</option><!--[-->`);
      ...
      _push(`</button></div><h3 class="text-3xl font-black text-slate-900 tracking-tight">Signal Intelligence</h3><p class="text-slate-500 font-medium">Real-time attribution and behavioral forensics</p></div><div class="flex items-center gap-3">`);
      ...
      ssrRenderList(siteHealth.value?.error_type_breakdown, (err) => {
        _push(`<div class="flex items-center justify-between"><span class="text-[10px] font-black text-slate-600 uppercase">${ssrInterpolate(err.type.replace("_", " "))}</span><div class="flex-1 mx-4 h-1.5 bg-slate-50 rounded-full overflow-hidden"><div class="h-full bg-indigo-500" style="${ssrRenderStyle({ width: siteHealth.value?.error_type_breakdown?.[0]?.count ? Math.min(100, err.count / siteHealth.value.error_type_breakdown[0].count * 100) + "%" : "0%" })}"></div></div><span class="text-[10px] font-black text-slate-900">${ssrInterpolate(err.count)}</span></div>`);
      });
      _push(`<!--]-->`);
      ...
      if (selectedSession.value) {
        ...
        ssrRenderList(sessionTimeline.value, (entry, idx) => {
          ...
          if (entry.click_count > 0) {
            _push(`<span class="${ssrRenderClass([{ "text-indigo-600": entry.click_count >= 3 }, "flex items-center gap-1.5 text-emerald-600 text-[9px] font-black uppercase tracking-widest"])}"><span class="${ssrRenderClass([entry.click_count >= 3 ? "bg-indigo-500" : "bg-emerald-500", "w-1.5 h-1.5 rounded-full animate-pulse"])}"></span> ${ssrInterpolate(entry.click_count)} Interactions `);
      ...
      if (showHealthModal.value && healthModalSite.value) {
        ...
        if (!isListening.value && !lastHeardSignal.value) {
          ...
          _push(`<div class="h-full flex flex-col items-center justify-center text-center p-8"><div class="w-32 h-32 bg-emerald-500 text-white rounded-full flex items-center justify-center text-5xl shadow-2xl shadow-emerald-200 mb-8 animate-bounce"> ✅ </div><h4 class="text-3xl font-black text-slate-900 tracking-tight mb-2">Connection Success!</h4><p class="text-lg text-emerald-600 font-black uppercase tracking-tight mb-8">Signal Received via Secure Handshake</p><div class="w-full bg-slate-900 rounded-3xl p-8 text-left space-y-4 border border-white/5"><div class="flex items-center justify-between"><span class="text-[10px] font-black text-slate-500 uppercase">Device</span><span class="text-xs font-bold text-white">${ssrInterpolate(lastHeardSignal.value.browser)} / ${ssrInterpolate(lastHeardSignal.value.platform)}</span></div><div class="flex items-center justify-between"><span class="text-[10px] font-black text-slate-500 uppercase">Source Path</span><span class="text-xs font-bold text-indigo-300 truncate max-w-[200px]">${ssrInterpolate(lastHeardSignal.value.page_url)}</span></div><div class="flex items-center justify-between"><span class="text-[10px] font-black text-slate-500 uppercase">Attribution ID</span><span class="text-xs font-bold text-emerald-400">${ssrInterpolate(lastHeardSignal.value.google_campaign_id || "System Default")}</span></div></div><button class="mt-8 text-[11px] font-black text-indigo-600 uppercase tracking-widest hover:underline">Run Another Test</button></div>`);
        } else {
          _push(`<!---->`);
        }
