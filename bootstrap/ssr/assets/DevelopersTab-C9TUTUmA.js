...
import { ssrRenderAttrs, ssrRenderClass, ssrInterpolate, ssrRenderAttr, ssrRenderList, ssrIncludeBooleanAttr, ssrRenderStyle, ssrRenderComponent, ssrLooseContain, ssrLooseEqual } from "vue/server-renderer";
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<svg class="${ssrRenderClass([{ "rotate-180": showSiteDropdown.value }, "w-3 h-3 transition-transform"])}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M19 9l-7 7-7-7"></path></svg></button><button class="px-3 py-2 text-slate-400 hover:text-white hover:bg-slate-900 rounded-xl transition-all"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 4v16m8-8H4"></path></svg></button></div>`);
      if (showSiteDropdown.value) {
        ...
        ssrRenderList(filteredSiteOptions.value, (site) => {
          ...
          if (selectedSiteId.value === site.id) {
            _push(`<span class="text-indigo-600"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg></span>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</button>`);
      ...
      if (selectedSite.value) {
        ...
        }, "flex items-center gap-2 px-5 py-3 rounded-2xl border font-black text-[11px] uppercase tracking-widest transition-all shadow-sm"])}"><span class="${ssrRenderClass([{
        ...
        }, "w-2 h-2 rounded-full"])}"></span> ${ssrInterpolate(pixelStatusBadge.value.label)}</div>`);
      } else {
        _push(`<div class="flex items-center gap-2 px-5 py-3 rounded-2xl border border-indigo-100 bg-indigo-50/50 text-indigo-600 font-black text-[11px] uppercase tracking-widest shadow-sm"><span class="w-2 h-2 rounded-full bg-indigo-500"></span> Aggregate Mode </div>`);
      }
      ...
      _push(`</div></div><div class="bg-indigo-600 p-8 shadow-indigo-200 shadow-2xl rounded-[2.5rem] text-white"><p class="text-[11px] font-black text-indigo-200 uppercase tracking-widest mb-1">Engagement Qty</p><h4 class="text-4xl font-black tracking-tight">${ssrInterpolate(avgClicks.value)}</h4><div class="mt-3 text-[10px] font-black text-indigo-200">Avg Clicks Per Ad Session</div></div></div><div class="lg:col-span-4 bg-white p-8 shadow-premium rounded-[2.5rem] border border-slate-100"><div class="flex items-center justify-between mb-6"><h3 class="text-xs font-black text-slate-900 uppercase tracking-widest">Pixel Health</h3><button${ssrIncludeBooleanAttr(isTestingConn.value) ? " disabled" : ""} class="text-[9px] font-black uppercase tracking-widest px-3 py-1.5 bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 transition-all">${ssrInterpolate(isTestingConn.value ? "Testing..." : "Test Now")}</button></div>`);
      if (selectedSite.value) {
        _push(`<div class="space-y-4"><div class="relative"><label class="text-[8px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1 block">Traffic Domain Pinning</label><input${ssrRenderAttr("value", allowedDomainInput.value)} placeholder="diaminyaesthetics.com" class="w-full bg-slate-50 border-slate-100 focus:border-indigo-500 focus:ring-0 rounded-xl text-[11px] font-bold text-slate-800 py-3 px-4 pr-16"><button class="absolute right-2 bottom-2 bg-slate-900 text-white text-[9px] font-black px-3 py-1.5 rounded-lg active:scale-95 transition-transform">Save</button></div><div class="flex items-center justify-between p-3 bg-slate-50/50 rounded-xl"><span class="text-[9px] font-black text-slate-400 uppercase">Subdomain Pinning</span><span class="text-[9px] font-black text-emerald-600 uppercase">Active (v3.1)</span></div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div></div><div class="bg-white p-8 shadow-premium rounded-[3rem] border border-slate-100"><div class="grid grid-cols-1 lg:grid-cols-12 gap-12"><div class="lg:col-span-8"><h4 class="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-6">Global Signal Origin</h4><div class="flex flex-wrap gap-4"><!--[-->`);
      ...
      _push(`</div><div class="overflow-x-auto"><table class="w-full text-left min-w-[900px]"><thead><tr class="bg-slate-50/60"><th class="py-5 px-12 text-[9px] font-black text-slate-400 uppercase tracking-widest">Page / Path</th><th class="py-5 px-6 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Intent Score</th><th class="py-5 px-6 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Total Hits</th><th class="py-5 px-6 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Engagement</th><th class="py-5 px-6 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">14-Day Trend</th><th class="py-5 px-10 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Δ vs Yesterday</th></tr></thead><tbody class="divide-y divide-slate-50">`);
      ...
      ssrRenderList(topPages.value, (page, idx) => {
        ...
        if (page.top_intent) {
          ...
          }, "px-2 py-0.5 text-[8px] font-black rounded-lg uppercase tracking-tighter border shadow-sm"])}">${ssrInterpolate(page.top_intent)} Intent</span>`);
        } else {
          _push(`<!---->`);
        }
        ...
        _push(`</div></div></div></td><td class="py-7 px-6 text-center"><div class="inline-flex flex-col items-center"><span class="${ssrRenderClass([page.engagement_score >= 70 ? "text-indigo-600" : "text-slate-900", "text-sm font-black"])}">${ssrInterpolate(page.engagement_score)}</span><div class="w-12 h-1 bg-slate-100 rounded-full mt-1 overflow-hidden"><div class="h-full bg-indigo-500" style="${ssrRenderStyle({ width: page.engagement_score + "%" })}"></div></div></div></td><td class="py-7 px-6 text-center"><span class="text-sm font-black text-slate-900">${ssrInterpolate(page.total_hits)}</span><p class="text-[9px] text-slate-400 font-bold mt-1">${ssrInterpolate(page.ad_hits)} Ad Hits</p></td><td class="py-7 px-6 text-center"><div class="flex flex-col items-center"><span class="text-xs font-black text-slate-700">${ssrInterpolate(page.avg_duration)}s Dwell</span><span class="text-[9px] text-slate-400 font-bold mt-1">${ssrInterpolate(page.avg_clicks)} Avg Clicks</span></div></td><td class="py-7 px-6"><div class="flex items-center justify-center"><svg width="80" height="28" class="overflow-visible"><defs><linearGradient${ssrRenderAttr("id", "sg" + idx)} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#6366f1" stop-opacity="0.3"></stop><stop offset="100%" stop-color="#6366f1" stop-opacity="0"></stop></linearGradient></defs>`);
      ...
      _push(`<!--]--></div></div></div><div class="grid grid-cols-1 lg:grid-cols-12 gap-8"><div class="lg:col-span-8 bg-slate-900 p-12 shadow-2xl rounded-[3.5rem] border border-slate-800 relative overflow-hidden"><div class="absolute -top-10 -right-10 w-64 h-64 bg-indigo-500/10 blur-[100px] rounded-full pointer-events-none"></div><div class="flex items-center justify-between mb-10"><h3 class="text-2xl font-black text-white flex items-center gap-4"><span class="p-3 bg-indigo-600 text-white rounded-2xl shadow-lg"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"></path></svg></span> Tracker Implementation: ${ssrInterpolate(selectedSite.value ? selectedSite.value.label : "Select a Site")}</h3><div class="flex items-center gap-3"><span class="px-3 py-1 bg-white/5 text-indigo-400 text-[9px] font-black rounded-lg border border-white/10 uppercase tracking-widest">v3.1 Secure Handshake</span></div></div><div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8"><div class="space-y-3"><label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Tracking Site</label><select class="w-full bg-slate-800 border-white/10 focus:border-indigo-500 focus:ring-0 rounded-2xl text-[11px] font-bold text-white py-4 px-6 appearance-none cursor-pointer"><option${ssrRenderAttr("value", null)} class="bg-slate-900 text-slate-400"${ssrIncludeBooleanAttr(Array.isArray(selectedSiteId.value) ? ssrLooseContain(selectedSiteId.value, null) : ssrLooseEqual(selectedSiteId.value, null)) ? " selected" : ""}>Select a Site...</option><!--[-->`);
      ssrRenderList(pixelSites.value, (site) => {
        _push(`<option${ssrRenderAttr("value", site.id)} class="bg-slate-900 text-white"${ssrIncludeBooleanAttr(Array.isArray(selectedSiteId.value) ? ssrLooseContain(selectedSiteId.value, site.id) : ssrLooseEqual(selectedSiteId.value, site.id)) ? " selected" : ""}>${ssrInterpolate(site.label)}</option>`);
      });
      _push(`<!--]--></select></div><div class="space-y-3"><label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Traffic Isolation</label><input${ssrRenderAttr("value", selectedCampaignId.value)} placeholder="e.g. MetaPilot_Agency_001" class="w-full bg-white/5 border-white/10 focus:border-indigo-500 focus:ring-0 rounded-2xl text-[11px] font-bold text-white py-4 px-6"></div><div class="space-y-3"><label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Property Link</label><select class="w-full bg-slate-800 border-white/10 focus:border-indigo-500 focus:ring-0 rounded-2xl text-[11px] font-bold text-white py-4 px-6 appearance-none cursor-pointer"><!--[-->`);
      ssrRenderList(__props.properties, (prop) => {
        _push(`<option${ssrRenderAttr("value", prop.id)} class="bg-slate-900 text-white"${ssrIncludeBooleanAttr(Array.isArray(selectedPropId.value) ? ssrLooseContain(selectedPropId.value, prop.id) : ssrLooseEqual(selectedPropId.value, prop.id)) ? " selected" : ""}>${ssrInterpolate(prop.name)}</option>`);
      });
      _push(`<!--]--></select></div></div><div class="mb-10"><label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 mb-4">Active Modules</label><div class="grid grid-cols-2 md:grid-cols-4 gap-4"><!--[-->`);
      ssrRenderList([
      ...
      _push(`</div></div><div class="space-y-8" style="${ssrRenderStyle(activeTab.value === "health" ? null : { display: "none" })}"><div class="bg-white shadow-premium rounded-[3.5rem] border border-slate-100/50 overflow-hidden"><div class="overflow-x-auto"><table class="w-full text-left border-collapse min-w-[1000px]"><thead><tr class="bg-slate-50/50"><th class="py-10 px-10 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">Error Discovery</th><th class="py-10 px-6 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">Origin / File</th><th class="py-10 px-6 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">Environment</th><th class="py-10 px-10 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 text-right">Timestamp</th></tr></thead><tbody class="divide-y divide-slate-50"><!--[-->`);
      ssrRenderList(errorResponse.value.data, (err) => {
        _push(`<tr class="group hover:bg-rose-50/30 transition-all"><td class="py-8 px-10"><div class="flex items-center gap-5"><div class="w-12 h-12 rounded-2xl border-2 border-slate-100 bg-white flex items-center justify-center text-rose-500 shadow-sm"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg></div><div class="max-w-md"><p class="text-xs font-black text-slate-900 line-clamp-2"${ssrRenderAttr("title", err.message)}>${ssrInterpolate(err.message)}</p><p class="text-[9px] text-slate-400 font-black uppercase tracking-tight mt-1 truncate">${ssrInterpolate(err.url)}</p></div></div></td><td class="py-8 px-6"><div><p class="text-xs font-black text-slate-800">${ssrInterpolate(err.filename ? err.filename.split("/").pop() : "Unknown Source")}</p>`);
        if (err.line) {
          _push(`<p class="text-[9px] text-slate-400 font-black uppercase tracking-widest">Line ${ssrInterpolate(err.line)}:${ssrInterpolate(err.col)}</p>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<span class="px-2 py-0.5 bg-slate-100 text-[8px] font-black text-slate-500 rounded-md uppercase tracking-wider mt-1 inline-block">${ssrInterpolate(err.source)}</span></div></td><td class="py-8 px-6"><div class="flex items-center gap-3"><div class="text-[9px] font-black text-slate-500 uppercase">${ssrInterpolate(err.user_agent ? err.user_agent.includes("Chrome") ? "Chrome" : err.user_agent.includes("Firefox") ? "Firefox" : "Other" : "Unknown")}</div><div class="w-1.5 h-1.5 bg-slate-200 rounded-full"></div><div class="text-[9px] font-black text-slate-400 uppercase line-clamp-1 max-w-[100px]">${ssrInterpolate(err.user_agent)}</div></div></td><td class="py-8 px-10 text-right"><p class="text-xs font-black text-slate-900">${ssrInterpolate(new Date(err.created_at).toLocaleTimeString())}</p><p class="text-[9px] text-slate-400 font-black uppercase mt-1">${ssrInterpolate(err.created_at.split("T")[0])}</p></td></tr>`);
      });
      _push(`<!--]--></tbody></table></div><div class="px-10 py-8 bg-slate-50/50 border-t border-slate-100 flex items-center justify-between"><p class="text-[10px] font-black text-slate-400 uppercase tracking-widest"> Showing <span class="text-slate-900">${ssrInterpolate(errorResponse.value.from || 0)}-${ssrInterpolate(errorResponse.value.to || 0)}</span> of <span class="text-slate-900">${ssrInterpolate(errorResponse.value.total)}</span> health signals </p><div class="flex items-center gap-3"><button${ssrIncludeBooleanAttr(errorFilters.value.page === 1 || isLoadingErrors.value) ? " disabled" : ""} class="px-5 py-2.5 bg-white border border-slate-200 rounded-xl text-[10px] font-black uppercase tracking-widest disabled:opacity-30 disabled:cursor-not-allowed hover:bg-slate-50 transition-all"> Prev </button><div class="flex items-center gap-2"><span class="text-[10px] font-black text-slate-900">Page ${ssrInterpolate(errorFilters.value.page)} of ${ssrInterpolate(errorResponse.value.last_page)}</span></div><button${ssrIncludeBooleanAttr(errorFilters.value.page === errorResponse.value.last_page || isLoadingErrors.value) ? " disabled" : ""} class="px-5 py-2.5 bg-white border border-slate-200 rounded-xl text-[10px] font-black uppercase tracking-widest disabled:opacity-30 disabled:cursor-not-allowed hover:bg-slate-50 transition-all"> Next </button></div></div>`);
      if (errorResponse.value.data.length === 0) {
        _push(`<div class="p-32 text-center"><div class="w-24 h-24 bg-emerald-50 rounded-[3rem] flex items-center justify-center mx-auto mb-10"><svg class="w-10 h-10 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg></div><h4 class="text-3xl font-black text-slate-900 mb-4 tracking-tighter uppercase italic">Optimal Health</h4><p class="text-slate-500 max-w-sm mx-auto font-medium">No client-side JavaScript errors or promise rejections captured in the field. Your frontend is running smoothly.</p></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div></div></div>`);
