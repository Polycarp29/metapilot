...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const topCountries = computed(() => {
      ...
      return Object.entries(c).map(([code, count]) => ({ code, count })).sort((a, b) => {
        if (a.code === "Unknown") return 1;
        if (b.code === "Unknown") return -1;
        return b.count - a.count;
      }).slice(0, 8);
    ...
    const hasCampaignData = computed(
      () => topCampaigns.value.length > 0 || sourceBreakdown.value.length > 0 || (gclidSummary.value?.total_gclid_hits ?? 0) > 0
    );
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(topCountries.value, (geo) => {
        ...
        if (geo.code !== "Unknown") {
          ...
          _push(`<div class="p-4 bg-slate-50/30 rounded-2xl border border-dashed border-slate-200 opacity-60 group" title="Visitors whose country could not be resolved — typically direct IPs not yet looked up by GeoIP"><div class="flex items-center justify-between mb-3"><span class="text-2xl">🌍</span><span class="text-[9px] font-black text-slate-300 uppercase tracking-widest">${ssrInterpolate(Math.round(geo.count / (analyticsData.value?.summary?.last30_hits || chartEvents.value.length || 1) * 100))}% </span></div><p class="text-[10px] font-black text-slate-400 leading-tight h-7">Unresolved</p><p class="text-[9px] font-black text-slate-400 uppercase mt-0.5">${ssrInterpolate(geo.count.toLocaleString())} <span class="text-slate-300">Signals</span></p></div>`);
        }
        _push(`<!--]-->`);
      });
      _push(`<!--]-->`);
      ...
      if (hasCampaignData.value) {
        _push(`<div class="bg-white shadow-premium rounded-[3.5rem] border border-slate-100/50 overflow-hidden"><div class="px-12 pt-12 pb-8 flex items-center justify-between border-b border-slate-50"><div><h3 class="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-4"> Campaign Attribution <span class="px-3 py-1 bg-amber-50 text-amber-600 text-[9px] font-black rounded-full uppercase tracking-widest border border-amber-100">30 Days</span></h3><p class="text-slate-400 text-xs font-medium mt-1">UTM campaign tracking · Google Ads GCLID attribution · Source / medium breakdown</p></div>`);
        if (gclidSummary.value?.total_gclid_hits > 0) {
          _push(`<div class="flex-shrink-0 flex flex-col items-center justify-center px-8 py-5 bg-gradient-to-br from-amber-400 to-orange-500 rounded-3xl text-white shadow-lg shadow-amber-200"><p class="text-[9px] font-black uppercase tracking-widest text-amber-100 mb-0.5">Google Ads Confirmed</p><p class="text-3xl font-black tracking-tight leading-none">${ssrInterpolate(gclidSummary.value.total_gclid_hits.toLocaleString())}</p><p class="text-[9px] font-bold text-amber-100 mt-1">${ssrInterpolate(gclidSummary.value.unique_gclids)} unique GCLIDs · ${ssrInterpolate(gclidSummary.value.unique_sessions)} sessions</p></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div>`);
        if (gclidSummary.value?.total_gclid_hits > 0) {
          _push(`<div class="grid grid-cols-2 md:grid-cols-4 gap-4 px-12 pt-8"><div class="p-5 bg-amber-50/50 rounded-2xl border border-amber-100/60"><p class="text-[9px] font-black text-amber-500 uppercase tracking-widest mb-1">Avg Dwell (Ads)</p><p class="text-2xl font-black text-slate-900">${ssrInterpolate(gclidSummary.value.avg_dwell)}<small class="text-sm font-bold text-slate-400 ml-1">s</small></p></div><div class="p-5 bg-amber-50/50 rounded-2xl border border-amber-100/60"><p class="text-[9px] font-black text-amber-500 uppercase tracking-widest mb-1">Avg Clicks (Ads)</p><p class="text-2xl font-black text-slate-900">${ssrInterpolate(gclidSummary.value.avg_clicks)}</p></div><div class="p-5 bg-indigo-50/50 rounded-2xl border border-indigo-100/60"><p class="text-[9px] font-black text-indigo-400 uppercase tracking-widest mb-1">Total Campaigns</p><p class="text-2xl font-black text-slate-900">${ssrInterpolate(topCampaigns.value.length)}</p></div><div class="p-5 bg-slate-50 rounded-2xl border border-slate-100"><p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Traffic Sources</p><p class="text-2xl font-black text-slate-900">${ssrInterpolate(sourceBreakdown.value.length)}</p></div></div>`);
        } else {
          _push(`<!---->`);
        }
        if (topCampaigns.value.length > 0) {
          _push(`<div class="px-12 pt-8 pb-4"><h4 class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Campaign Performance</h4><div class="overflow-x-auto"><table class="w-full text-left min-w-[700px]"><thead><tr class="bg-slate-50/60"><th class="py-4 px-6 text-[9px] font-black text-slate-400 uppercase tracking-widest rounded-l-2xl">Campaign</th><th class="py-4 px-6 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Source / Medium</th><th class="py-4 px-6 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Total Hits</th><th class="py-4 px-6 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">GCLID Hits</th><th class="py-4 px-6 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Avg Dwell</th><th class="py-4 px-6 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center rounded-r-2xl">Avg Clicks</th></tr></thead><tbody class="divide-y divide-slate-50"><!--[-->`);
          ssrRenderList(topCampaigns.value, (camp, idx) => {
            _push(`<tr class="group hover:bg-slate-50/80 transition-all"><td class="py-5 px-6"><div class="flex items-center gap-3"><div class="${ssrRenderClass([idx === 0 ? "bg-amber-500" : idx === 1 ? "bg-indigo-500" : "bg-slate-300", "w-7 h-7 rounded-xl flex items-center justify-center text-[10px] font-black text-white"])}">${ssrInterpolate(idx + 1)}</div><div class="min-w-0"><p class="text-[11px] font-black text-slate-900 truncate max-w-[180px]"${ssrRenderAttr("title", camp.campaign)}>${ssrInterpolate(camp.campaign)}</p></div></div></td><td class="py-5 px-6 text-center"><div class="inline-flex flex-col items-center gap-1">`);
            if (camp.source) {
              _push(`<span class="px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded-md text-[9px] font-black border border-indigo-100">${ssrInterpolate(camp.source)}</span>`);
            } else {
              _push(`<!---->`);
            }
            if (camp.medium) {
              _push(`<span class="px-2 py-0.5 bg-slate-100 text-slate-500 rounded-md text-[9px] font-black">${ssrInterpolate(camp.medium)}</span>`);
            } else {
              _push(`<!---->`);
            }
            if (!camp.source && !camp.medium) {
              _push(`<span class="text-[9px] text-slate-300 italic">—</span>`);
            } else {
              _push(`<!---->`);
            }
            _push(`</div></td><td class="py-5 px-6 text-center"><span class="text-sm font-black text-slate-900">${ssrInterpolate(camp.hits.toLocaleString())}</span></td><td class="py-5 px-6 text-center">`);
            if (camp.gclid_hits > 0) {
              _push(`<span class="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-50 text-amber-700 rounded-lg text-[10px] font-black border border-amber-100"><svg class="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"></path><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"></path><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"></path><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"></path></svg> ${ssrInterpolate(camp.gclid_hits.toLocaleString())}</span>`);
            } else {
              _push(`<span class="text-[9px] text-slate-300 font-bold">—</span>`);
            }
            _push(`</td><td class="py-5 px-6 text-center"><span class="text-[11px] font-black text-slate-700">${ssrInterpolate(camp.avg_dwell)}s</span></td><td class="py-5 px-6 text-center"><span class="text-[11px] font-black text-slate-700">${ssrInterpolate(camp.avg_clicks)}</span></td></tr>`);
          });
          _push(`<!--]--></tbody></table></div></div>`);
        } else {
          _push(`<!---->`);
        }
        if (sourceBreakdown.value.length > 0) {
          _push(`<div class="px-12 pt-6 pb-10 border-t border-slate-50 mt-4"><h4 class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Traffic Sources</h4><div class="flex flex-wrap gap-3"><!--[-->`);
          ssrRenderList(sourceBreakdown.value, (src) => {
            _push(`<div class="group flex items-center gap-3 px-4 py-2.5 bg-slate-50 hover:bg-indigo-50 rounded-2xl border border-slate-100 hover:border-indigo-200 transition-all cursor-default"><div class="flex flex-col"><span class="text-[10px] font-black text-slate-800">${ssrInterpolate(src.source)} `);
            if (src.medium) {
              _push(`<span class="text-slate-400 font-bold"> / ${ssrInterpolate(src.medium)}</span>`);
            } else {
              _push(`<!---->`);
            }
            _push(`</span></div><span class="px-2.5 py-1 bg-white text-indigo-600 rounded-lg text-[9px] font-black border border-indigo-100 shadow-sm">${ssrInterpolate(src.hits.toLocaleString())}</span>`);
            if (src.gclid_hits > 0) {
              _push(`<span class="w-5 h-5 rounded-full bg-amber-400 flex items-center justify-center text-[8px] font-black text-white" title="Google Ads confirmed"> G </span>`);
            } else {
              _push(`<!---->`);
            }
            _push(`</div>`);
        ...
        if (!topCampaigns.value.length && !sourceBreakdown.value.length) {
          _push(`<div class="px-12 py-16 text-center"><p class="text-slate-300 text-[11px] font-black uppercase tracking-widest italic">No campaign parameters detected yet — add UTM tags or enable Google Ads auto-tagging on your campaigns.</p></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div>`);
      ...
      ssrRenderList(topPages.value, (page, idx) => {
        ...
        if (!page.matched_keywords.length) {
          _push(`<span class="text-[9px] text-slate-300 font-bold truncate max-w-xs">${ssrInterpolate(safeHostname(page.display_url))}</span>`);
        } else {
          _push(`<!---->`);
        }
        ...
        _push(`</svg></div></td><td class="py-7 px-10 text-right"><span class="${ssrRenderClass([deltaBadgeClass(page.delta_pct), "inline-flex items-center px-3 py-1.5 rounded-xl text-[10px] font-black font-mono shadow-sm"])}">${ssrInterpolate(deltaIcon(page.delta_pct))}</span></td></tr>`);
        if (page.showRecs) {
          ...
          _push(`<div class="mt-6 flex items-center gap-4"><button${ssrIncludeBooleanAttr(injectingPage.value === page.display_url) ? " disabled" : ""} class="px-4 py-2 bg-indigo-600 text-white text-[9px] font-black rounded-lg uppercase tracking-tight shadow-md hover:bg-indigo-500 transition-all active:scale-95 disabled:opacity-50">${ssrInterpolate(injectingPage.value === page.display_url ? "Injecting..." : "Auto-Inject Schema")}</button><button${ssrIncludeBooleanAttr(fetchingSource.value === page.display_url) ? " disabled" : ""} class="px-4 py-2 bg-white text-slate-600 border border-slate-200 text-[9px] font-black rounded-lg uppercase tracking-tight hover:bg-slate-50 transition-all active:scale-95 disabled:opacity-30">${ssrInterpolate(fetchingSource.value === page.display_url ? "Fetching..." : "View Page Source")}</button></div></div></div></div></td></tr>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<!--]-->`);
      ...
      ssrRenderList(rising.value, (page) => {
        _push(`<div class="flex items-center justify-between p-5 bg-emerald-50/50 hover:bg-emerald-50 rounded-2xl border border-emerald-100/50 cursor-pointer transition-all group"><div class="min-w-0 mr-4"><p class="text-xs font-black text-slate-900 truncate group-hover:text-emerald-700 transition-colors">${ssrInterpolate(safePathLabel(page.display_url))}</p><p class="text-[9px] text-slate-400 font-bold mt-0.5">${ssrInterpolate(page.last7)} hits this week</p></div><span class="shrink-0 text-[11px] font-black text-emerald-600 bg-white px-3 py-1.5 rounded-xl border border-emerald-200 shadow-sm"> ↑ +${ssrInterpolate(page.delta_pct)}% </span></div>`);
      });
      ...
      ssrRenderList(falling.value, (page) => {
        _push(`<div class="flex items-center justify-between p-5 bg-rose-50/30 hover:bg-rose-50/60 rounded-2xl border border-rose-100/50 cursor-pointer transition-all group"><div class="min-w-0 mr-4"><p class="text-xs font-black text-slate-900 truncate group-hover:text-rose-700 transition-colors">${ssrInterpolate(safePathLabel(page.display_url))}</p><p class="text-[9px] text-slate-400 font-bold mt-0.5">${ssrInterpolate(page.last7)} hits this week</p></div><span class="shrink-0 text-[11px] font-black text-rose-600 bg-white px-3 py-1.5 rounded-xl border border-rose-200 shadow-sm"> ↓ ${ssrInterpolate(page.delta_pct)}% </span></div>`);
      });
      ...
      _push(`</div></div></div><div class="flex items-center gap-4"><button class="px-5 py-3 bg-white/5 border border-white/10 text-white text-[10px] font-black rounded-2xl flex items-center gap-2 hover:bg-white/10 transition-all shadow-xl"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 4v16m8-8H4"></path></svg> New Identification </button></div></div><div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8"><div class="space-y-3"><label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Lead Attribution ID</label><input${ssrRenderAttr("value", selectedCampaignId.value)} placeholder="e.g. fb_ads_winter_campaign" class="w-full bg-white/5 border-white/10 focus:border-indigo-500 focus:ring-0 rounded-2xl text-[11px] font-bold text-white py-4 px-6"></div><div class="space-y-3"><div class="flex items-center justify-between ml-1"><label class="text-[10px] font-black text-slate-500 uppercase tracking-widest">Identification Source</label>`);
      if (selectedSite.value) {
        _push(`<div class="flex items-center gap-2"><span class="${ssrRenderClass([selectedSite.value.tracking_enabled ? "text-emerald-500" : "text-rose-500", "text-[9px] font-black uppercase tracking-widest"])}">${ssrInterpolate(selectedSite.value.tracking_enabled ? "Ingestion Active" : "Ingestion Paused")}</span><button${ssrIncludeBooleanAttr(isTogglingTracking.value) ? " disabled" : ""} class="${ssrRenderClass([[selectedSite.value.tracking_enabled ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.4)]" : "bg-slate-700", isTogglingTracking.value ? "opacity-50" : ""], "w-8 h-4 rounded-full transition-all relative flex items-center px-0.5"])}"><div class="${ssrRenderClass([{ "translate-x-4": selectedSite.value.tracking_enabled }, "w-3 h-3 bg-white rounded-full transition-all"])}"></div></button></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div><select class="w-full bg-slate-800 border-white/10 focus:border-indigo-500 focus:ring-0 rounded-2xl text-[11px] font-bold text-white py-4 px-6 appearance-none cursor-pointer"><option${ssrRenderAttr("value", null)}${ssrIncludeBooleanAttr(Array.isArray(selectedSiteId.value) ? ssrLooseContain(selectedSiteId.value, null) : ssrLooseEqual(selectedSiteId.value, null)) ? " selected" : ""}>System Selection...</option><!--[-->`);
